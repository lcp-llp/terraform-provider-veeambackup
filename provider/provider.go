	...
	return &schema.Provider{
		Schema: map[string]*schema.Schema{
			...
			"api_key": {
				...
				DefaultFunc: schema.EnvDefaultFunc("VEEAM_API_KEY", nil),
			},
			"username": {
				Type:        schema.TypeString,
				Required:    true,
		...
		DataSourcesMap: map[string]*schema.Resource{
			"veeam_azure_backup_repositories": dataSourceAzureBackupRepositories(),
			"veeam_azure_backup_repository":   dataSourceAzureBackupRepository(),
			"veeam_azure_service_accounts":    dataSourceAzureServiceAccounts(),
			"veeam_azure_service_account":     dataSourceAzureServiceAccount(),
		},
